...
in
{
  ...
  programs.helix = {
    ...
    themes.stylix-override = pkgs.writeText "stylix-override.toml" ''
      inherits = "stylix"
      "ui.background" = { bg = "${bg}" }
      "ui.window" = { bg = "${bg}" }
      "ui.gutter" = { bg = "${bg}" }
      ...
      "ui.linenr" = { bg = "${bg}" }
      "ui.linenr.selected" = { bg = "${bg}" }
    '';
    settings = {
      theme = lib.mkForce "stylix-override";
      editor = {
        popup-border = "all";
          ...
          config = {
            ...
            languages =
              lib.genAttrs
                ...
                (_: [
                  {
                    formatCommand = "${pkgs.prettier}/bin/prettier --stdin-filepath \${INPUT}";
                    formatStdin = true;
                  }
                ]);
          };
